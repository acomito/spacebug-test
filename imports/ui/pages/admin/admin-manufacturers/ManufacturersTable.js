...
const columns = [
	{
	  ...
	  dataIndex: '_id',
	  key: '_id',
	  render: _id => <Link to={`/admin/manufacturers/${_id}`}>{_id}</Link>,
	},
	{
	  title: 'title',
	  dataIndex: 'title',
...
class ManufacturersTable extends React.Component {
	render(){
		...
		return (
			<Table
				...
				columns={columns} 
				dataSource={this.props.data.manufacturers}  
			/>
		);
	}
}

export default graphql(GET_MANUFACTURERS)(ManufacturersTable);